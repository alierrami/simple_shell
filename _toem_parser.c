 ...
 */
int _is_cmd(_info_t *info, char *path)
{
	struct stat st;
...
/**
 * __find_path - finds this cmd in the PATH string
 * @info: the info struct
 * @pathstr: the PATH string
 * @cmd: the cmd to find
 *
 ...
 */
char *__find_path(_info_t *info, char *pathstr, char *cmd)
{
	int i = 0, curr_pos = 0;
	char *path;
	...
	if (!pathstr)
		return (NULL);
	if ((_str_len(cmd) > 2) && __starts_with(cmd, "./"))
	{
		if (_is_cmd(info, cmd))
			return (cmd);
	}
